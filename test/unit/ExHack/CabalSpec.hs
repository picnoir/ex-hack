...
import           ExHack.Types             (ComponentRoot (..), Package (..),
                                           PackageComponent (..),
                                           PackageIdentifier (..),
                                           PackageDesc (..), PackageFilePath(..), fromComponents,
                                           mkVersion)
...
spec = describe "parseCabalFile" $
          it "should parse a file" $ do
            ...
            parseCabalFile (PackageDesc (PackageFilePath "fake", cf)) `shouldBe` (Just $ Package (PackageIdentifier "ad" $ mkVersion [4, 3, 5]) (fromList ["array", "base", "comonad", "containers", "data-reify", "erf", "free", "nats", "reflection", "semigroups", "transformers", "directory", "doctest", "filepath", "criterion"]) cf (PackageFilePath "fake") exposedMods Nothing allMods)
